...
const { writeFileSync, readFileSync, createWriteStream, unlinkSync, renameSync } = require("fs");
...
const convertToMp3 = async (track) => {
  ...
  const tmpMp3FileName = `${CONVERTED_DIR}/${track.id}.mp3`;
  ...
  await ffmpeg()
    ...
    .outputFormat('mp3')
    .on("error", (error) => console.log('Encoding Error 1', error))
    .on("end", () => {
...
app.get("/deleteTemps", (req, res) => {
  try {
    ...
    (tracks || []).forEach(async (track) => {
      if (track.file) {
        exec(`rm ${CONVERTED_DIR}/${track.id}.mp3`, {cwd: './'});
      }
    });
